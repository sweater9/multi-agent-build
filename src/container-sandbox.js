...
export function cleanRegistryUrl(value){const raw=String(value||'').trim();if(!raw)throw new Error('SANDBOX_NPM_REGISTRY_URL is required');let url;try{url=new URL(raw)}catch{throw new Error('SANDBOX_NPM_REGISTRY_URL is invalid')}if(!['http:','https:'].includes(url.protocol)||url.username||url.password||raw.length>300)throw new Error('SANDBOX_NPM_REGISTRY_URL is invalid');url.pathname=url.pathname.replace(/\/+$/,'')+'/';url.search='';url.hash='';return url.toString()}
...
export function dockerArgs({workspace,image='node:20-alpine',command,network='off',registryNetwork=null,registryUrl=null,memoryMb=768,timeoutMs=180000,user=defaultUser(),readOnlyRoot=true,shmMb=128}){
  ...
  else if(network==='registry-only'){
    if(!registryNetwork)throw new Error('registry-only network requires an isolated registry network');
    const npmRegistry=cleanRegistryUrl(registryUrl);
    args.push('--network',String(registryNetwork),'--env',`npm_config_registry=${npmRegistry}`,'--env','npm_config_audit=false','--env','npm_config_fund=false');
