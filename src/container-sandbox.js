...
export function dockerArgs({workspace,image='node:20-alpine',command,network='off',memoryMb=768,timeoutMs=180000}){
  const args=['run','--rm','--init','--cap-drop=ALL','--security-opt','no-new-privileges','--pids-limit','256','--memory',`${Math.max(128,Math.min(Number(memoryMb)||768,2048))}m`,'--cpus','1.5','--workdir','/workspace','--mount',`type=bind,src=${workspace},dst=/workspace`,'--tmpfs','/tmp:rw,noexec,nosuid,size=128m'];
...
export class ContainerSandboxExecutor{
  constructor({engine='docker',image='node:20-alpine',spawnImpl=spawn,tmpRoot=os.tmpdir()}={}){if(!['docker','podman'].includes(engine))throw new Error('Sandbox engine must be docker or podman');this.engine=engine;this.image=image;this.spawnImpl=spawnImpl;this.tmpRoot=tmpRoot}
  async execute({files,plan,project_id='project',attempt=0}={}){const clean=safeFiles(files),root=await fs.mkdtemp(path.join(this.tmpRoot,'multi-agent-sandbox-'));const workspace=path.join(root,'workspace');await fs.mkdir(workspace,{recursive:true,mode:0o700});try{for(const file of clean){const target=path.join(workspace,file.path);await fs.mkdir(path.dirname(target),{recursive:true,mode:0o700});await fs.writeFile(target,file.content,{mode:0o600})}const results=[];for(const step of Array.isArray(plan?.steps)?plan.steps:[]){const {args,timeoutMs}=dockerArgs({workspace,image:this.image,command:step.command,network:step.network,memoryMb:plan?.limits?.memory_mb,timeoutMs:step.timeout_ms||plan?.limits?.timeout_ms});const result=await spawnCapture(this.engine,args,{timeoutMs,maxOutputBytes:plan?.limits?.max_output_bytes||256000,spawnImpl:this.spawnImpl});results.push({name:String(step.name||'step'),...result});if(result.status!=='success')break}return{passed:results.every(step=>step.status==='success'),project_id,attempt,steps:results}}finally{await fs.rm(root,{recursive:true,force:true})}}
}
