...
function cleanSpecialists(value) {
  ...
  for (const item of value.slice(0, 4)) {
    ...
    seen.add(id); out.push({ id, name, focus });
  }
...
export class DynamicPromptWorkspace {
  constructor({ provider, stateStore = null, clock = () => new Date() } = {}) {
    if (!provider?.generate) throw new Error('provider is required');
    this.provider = provider; this.stateStore = stateStore; this.clock = clock;
  }
  ...
  async execute(goal, options = {}) {
    ...
    try {
      ...
      run.final_synthesized_result = { approved, summary: approved ? 'Dynamic specialist workflow completed.' : 'Final QA blocked the result.', answer: text(qa?.answer || judge?.answer || judge), quality_score: Number(qa?.quality_score || 0), confidence: Number(judge?.confidence || 0), findings: qa?.findings || [] };
      await this.save(run);
      return {
      ...
      run.state = 'failed'; run.workflow_status = 'failed'; run.final_synthesized_result = { approved: false, summary: 'Dynamic workflow failed safely.', error: String(error?.message || error) }; await this.save(run); throw error;
    }
  }
}
